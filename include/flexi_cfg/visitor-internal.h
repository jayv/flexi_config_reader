  ...
  switch (cfg_val->type) {
    ...
    case config::types::Type::kNumber: {
      ...
      if (config != nullptr) {
        if constexpr (visitor::IntValueVisitor<Visitor>) {
          if (int iVal; tryCast(config->value_any, iVal)) {
            visitor.onValue(static_cast<int64_t>(iVal));
            break;
          }
          if (int64_t llVal; tryCast(config->value_any, llVal)) {
            visitor.onValue(llVal);
            break;
          }
        }
        if constexpr (visitor::FloatValueVisitor<Visitor>) {
          if (float fVal; tryCast(config->value_any, fVal)) {
            visitor.onValue(fVal);
            break;
          }
          if (double dVal; tryCast(config->value_any, dVal)) {
            visitor.onValue(dVal);
            break;
          }
        }
      }
    ...
    case config::types::Type::kBoolean: {
      if constexpr (visitor::BoolValueVisitor<Visitor>) {
        auto config = std::dynamic_pointer_cast<config::types::ConfigValue>(cfg_val);
        if (bool bVal; config != nullptr && tryCast(config->value_any, bVal)) {
          visitor.onValue(bVal);
          break;
        }
      }
    }
