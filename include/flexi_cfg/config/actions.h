...
#define VERBOSE_DEBUG_ACTIONS 1  // NOLINT(cppcoreguidelines-macro-usage, *-macro-to-enum)
...
struct action<BEGIN> {
  template <typename ActionInput>
  static void apply(const ActionInput& in, ActionData& out) {
    logger::info("Begin file: {}", out.includes.top().file);
  }
};
...
struct action<END> {
  template <typename ActionInput>
  static void apply(const ActionInput& in, ActionData& out) {
    logger::info("End file: {}", out.includes.top().file);
  }
};
...
struct action<TAIL> {
  template <typename ActionInput>
  static void apply(const ActionInput& in, ActionData& out) {
    logger::trace("TailGrabber [{}:{}]:  [{}]", in.position().source, in.position().line, in.string());
  }
};
